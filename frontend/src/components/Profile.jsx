...
import { getUsers, getSkills, updateProfile, getTickets, getWorkLogs } from '../api';
import { Edit3, Briefcase, Ticket, Clock, CheckCircle2, Target, FolderKanban, Star, X, ChevronLeft, ChevronRight, Send, Gitlab, Github, Megaphone, Share2, Phone, Mail, Code, Bug, Palette, Headphones, Users as UsersIcon } from 'lucide-react';
...
const Profile = () => {
    ...
    const isOwnProfile = !id || (loggedInUser && parseInt(id) === loggedInUser.id);
    const targetUserId = id ? parseInt(id) : loggedInUser?.id;

    const fetchData = async () => {
        try {
            const [usersRes, skillsRes, tRes, lRes] = await Promise.all([
                getUsers(), getSkills(), getTickets(), getWorkLogs()
            ]);
            ...
            const userTickets = allTickets.filter(t => t.assignee === targetUserId);
            const userLogs = allLogs.filter(l => l.user === targetUserId);

            setTickets(userTickets.filter(t => ['OPEN', 'IN_PROGRESS', 'REVIEW'].includes(t.status)));
            setAnalyticsData({ tickets: userTickets, logs: userLogs });

            const todayStr = new Date().toDateString();
            const currentMonth = new Date().getMonth();
            const currentYear = new Date().getFullYear();

            const my_worked_minutes_today = userLogs
                .filter(l => new Date(l.created_at).toDateString() === todayStr)
                .reduce((acc, l) => acc + l.time_spent_minutes, 0);

            const my_completed_tasks_month = userTickets
                .filter(t => t.status === 'DONE' && new Date(t.updated_at).getMonth() === currentMonth && new Date(t.updated_at).getFullYear() === currentYear)
                .length;

            const my_assigned_count = userTickets.filter(t => ['OPEN', 'IN_PROGRESS', 'REVIEW'].includes(t.status)).length;

            setStats({ my_assigned_count, my_worked_minutes_today, my_completed_tasks_month });
            ...
            if (isOwnProfile) {
                setEditForm({
                    ...
                    tg_link: targetUser.profile?.tg_link || '',
                    tg_channel_link: targetUser.profile?.tg_channel_link || '',
                    gitlab_link: targetUser.profile?.gitlab_link || '',
                    github_link: targetUser.profile?.github_link || ''
                });
            }
        } catch (e) { console.error("Ошибка загрузки профиля", e); }
    };

    useEffect(() => { fetchData(); }, [activeTab, targetUserId]);
    ...
    const canSeeRole = loggedInUser?.profile?.role === 'ADMIN' || loggedInUser?.profile?.role === 'GENERAL_MANAGER';
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '2rem' }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', color: '#6b7280', fontSize: '0.95rem' }}>
                            ...
                            {canSeeRole && (
                                <span style={{ paddingLeft: '1rem', borderLeft: '2px solid #e5e7eb' }}>Роль: {user.profile?.role === 'ADMIN' ? 'Администратор' : 'Главный менеджер'}</span>
                            )}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem', marginTop: '1rem' }}>
                            {user.profile?.departments_details?.map(link => {
                                ...
                                return (
                                    <div key={link.id} style={{ display: 'inline-flex', alignItems: 'stretch', background: level.bg, border: `1px solid ${level.color}30`, borderRadius: '10px', color: level.color, fontSize: '0.85rem', fontWeight: '600', overflow: 'hidden' }}>
                                        <div style={{ background: level.color, color: 'white', padding: '0.4rem 0.8rem', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'flex', alignItems: 'center' }}>
                                            {level.label}
                                        </div>
                                        <div style={{ padding: '0.4rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {activeTab === 'tasks' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    {availableMonths.length === 0 ? (
                        <div className="biz-card" style={{ textAlign: 'center', color: '#9ca3af', padding: '4rem' }}>Пока не было задач.</div>
                    ) : (
                        <>

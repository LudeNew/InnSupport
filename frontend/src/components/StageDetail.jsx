...
import { ArrowLeft, CheckCircle2, ListTodo, Clock, Target, Palette, Code, Bug, Rocket, Layout, XCircle } from 'lucide-react';
...
const StageDetail = () => {
    ...
    return (
        <div style={{ padding: '0 0.5rem 2rem 0.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            {/* ЭТАЛОННАЯ ШАПКА, АДАПТИРОВАННАЯ ПОД ЭТАП */}
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem' }}>
                <button
                    onClick={() => navigate(`/projects/${stage.project}`)}
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.9rem', color: '#6b7280', marginBottom: '0.5rem', fontWeight: '500' }}>
                        Проект: <span style={{ color: '#4f46e5' }}>{stage.project_name}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: '8px', background: '#e0e7ff', color: '#4f46e5' }}>
            ...
            {/* СПИСКИ (Либо задачи, либо графики) */}

            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                {filteredProjects.map(project => (
                    ...
                    >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flex: 2 }}>
                            ...
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '0.4rem' }}>
                                    ...
                                    {/* Метка Внутренний / Внешний */}
                                    <span style={{ fontSize: '0.7rem', fontWeight: 'bold', padding: '0.2rem 0.6rem', borderRadius: '12px', background: project.project_type === 'INTERNAL' ? '#e0e7ff' : '#fce7f3', color: project.project_type === 'INTERNAL' ? '#4f46e5' : '#db2777' }}>
                                        {project.project_type === 'INTERNAL' ? 'Внутренний' : 'Внешний'}
                                    </span>
                                </div>
                        ...
                        {/* Метрики проекта (Меняются для закрытых) */}
                        <div style={{ display: 'flex', gap: '3rem', flex: 2, justifyContent: 'center' }}>
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>
                                    <LayoutList size={18} color="#8b5cf6" />
                                    {project.status === 'CLOSED' ? (project.total_stages_count || 0) : (project.active_stages_count || 0)}
                                </div>
                                <div style={{ fontSize: '0.8rem', color: '#6b7280', marginTop: '0.2rem' }}>
                                    {project.status === 'CLOSED' ? 'Всего этапов' : 'Активные этапы'}
                                </div>
                            </div>
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>
                                    <Target size={18} color="#ef4444" />
                                    {project.status === 'CLOSED' ? (project.total_tickets_count || 0) : (project.open_tickets_count || 0)}
                                </div>
                                <div style={{ fontSize: '0.8rem', color: '#6b7280', marginTop: '0.2rem' }}>
                                    {project.status === 'CLOSED' ? 'Всего задач' : 'Открыто задач'}
                                </div>
                            </div>
                            <div style={{ textAlign: 'center' }}>
                                <div style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>

...
const SubNavigation = ({ location }) => {
    ...
    const subsections = {
        ...
        '/projects': [{ label: 'Внутренние', tab: 'internal' }, { label: 'Внешние', tab: 'external' }, { label: 'Все', tab: 'all' }],
        ...
        // ДОБАВЛЕНА ВКЛАДКА "ЗАДАЧИ" В ПРОФИЛЬ
    ...
    return (
        <div className="top-bar-container" style={{ position: 'relative' }}>
            <h2 style={{ margin: 0 }}>
                {path === '/' ? 'Дашборд' : path === '/projects' ? 'Проекты' : path === '/tickets' ? 'Задачи' : path === '/team' ? 'Команда' : path === '/reports' ? 'Отчеты' : path === '/profile' ? 'Личный кабинет' : 'Раздел'}
            </h2>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                <div onClick={() => navigate('/profile')} style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', background: 'white', padding: '0.4rem 0.8rem 0.4rem 0.4rem', borderRadius: '99px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', cursor: 'pointer' }}>
                    {avatarUrl ? (
                        ...
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#111827', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 'bold' }}>{userInitial}</div>
                    )}
